...
//              return false;
        ...
        strictEqual(requestQueue.requests.length, 3, "has added only the queued requests to the queue");

        tracker.disableQueueRequest();
        strictEqual(false, requestQueue.enabled);
        
        tracker.queueRequest('myQueueDisabled=bar&queue=4');
        strictEqual(requestQueue.requests.length, 3, "does not increase number of queued requests but send it directly");
